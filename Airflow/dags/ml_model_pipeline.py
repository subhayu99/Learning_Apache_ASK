...
def getClient(connection_url: str):
    try:
...
def main():
    ...
    client = getClient('mongodb://localhost:27017/')
    ...
    df = getDF(data)
    df = dateTimeformat(df) # data extraction
    df = getNonDenied(df)
    ...
    df1 = encoding(df,col) 
    X,Y = split(df1) # data preprocessing
    X = X.drop([
        "Date_",
#        "open_product_food_drink",
#        "tip",
    ],axis=1)
    x_train,x_test, y_train, y_test = train_test_split(X,Y, test_size=0.2, random_state=0)
    print(x_train, y_train)
    regr = training(x_train,y_train,x_test,y_test)
